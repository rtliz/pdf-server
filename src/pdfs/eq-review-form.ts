        ...
        [
          {
            table: {
              body: [
                [
                  data.Logo ? logoProject(data.Logo) : null, logoAP(),
                ],
              ],
            },
            layout: {
...
function formDescription(collection) {
  ...
  if (part1 && part5) {
    part1?.Questions?.find(f => f.CRMInterface === 'title_name')?.Answers?.forEach(m => {
      ...
      result[0].push(textLabel(m.AnswerText, null, [0, 2, 0, 0]));
  ...
  return [
    layoutFormDescription(result[0], widthLine0, [-5, -10, 0, 0]),
    layoutFormDescription(result[1], ['auto', '*', 'auto', '*', 'auto', '*'], [-5, -5, 0, 0]),
    layoutFormDescription(result[2], ['auto', '*', 'auto', '*', 'auto', '*'], [-5, 0, 0, 0]),
    layoutFormDescription(result[3], ['auto', '*', 'auto', '*', 'auto', '*'], [-5, 0, 0, 0]),
    layoutFormDescription(result[4], ['auto', 'auto', '*'], [-5, 0, 0, 0]),
    layoutFormDescription(result[5], ['auto', '*', 'auto', '*', 'auto', '*'], [-5, -6, 0, 5]),
  ]
}
        ...
        [
          {
            ...
            fillColor: PaperColorHeader,     // สีพื้นหลัง
...
function getTextWidth(text, font = '7.5px AP') {
...
function genarateAnswers(data, part2Questions?, CRMInterface?) {
  let maxColumnsPerRow = data?.PaperLengthChoiceConfigNo ?? 4; // จำนวน column สูงสุดต่อ row
  let maxColumnWidth = 100; // ความกว้างสูงสุดต่อ column
  if (data?.PaperLengthChoiceConfigNo == 4) {
    maxColumnWidth = 100;
  } else if (data?.PaperLengthChoiceConfigNo == 3) {
    maxColumnWidth = 126;
  } else if (data?.PaperLengthChoiceConfigNo == 2) {
    maxColumnWidth = 190;
  }
  let occupationContent = [];
  let currentRow = [];
  data?.Answers?.forEach((m, i) => {
    let approxWidth = getTextWidth(m.AnswerText);

    let widthCol = ['auto', '*']
    let body: any = [checkbox(2), { text: m.AnswerText, margin: [0, 0, 0, 0] }]
    let layout: any = {
      ...setting.layoutTableBorder,
    }
    if (m.AnswerType == 2) {
      let answerOther = []
      answerOther.push(textLabel(m.AnswerText, null, [0, 0, 0, -4]))
      answerOther.push(borderBottom())
      approxWidth = getTextWidth(m.AnswerText, 'AP') + 20;
      body = [checkbox(2, 2), layoutFormDescription(answerOther, ['auto', '*'], [-1, -2, 0, 0])]
      layout = {
        ...setting.layoutTableEmpty,
        hLineStyle: function () {
          return { dash: { length: 0.2, space: 2 } }; // เส้นประ
        }
      }
    ...
    if (CRMInterface == "marital_status" && m.AnswerText.includes("สมรส") && !m.AnswerText.includes("ไม่มีบุตร")) {
      let children_numb = part2Questions?.find(f => f.CRMInterface === "children_numb")
      let answerOther = []
      answerOther.push(textLabel(m.AnswerText + " " + children_numb?.QuestionTitle, null, [0, 0, 0, -4]))
      answerOther.push(borderBottom())
      answerOther.push(textLabel("คน", null, [0, 0, 0, -4]))
      body = [checkbox(2), layoutFormDescription(answerOther, ['auto', 30, 'auto'], [-3, -3, 0, 0])]
      approxWidth = getTextWidth(m.AnswerText + " " + children_numb?.QuestionTitle + " คน", 'AP') + 40;
    }
    ...
    approxWidth += 10 // + checkbox width + margin

    if (approxWidth > maxColumnWidth) {
      // ความยาว ข้อความ หาร maxColumnWidth จะได้ค่า colSpan
      col["colSpan"] = (Math.floor(approxWidth / maxColumnWidth) || 1) + 1

      if (col["colSpan"] + currentRow.length >= maxColumnsPerRow) {
        col["colSpan"] = (Math.floor(approxWidth / maxColumnWidth) || 1) + 1
        if (currentRow.length + col["colSpan"] > maxColumnsPerRow) {
          // ถ้า colSpan มากกว่า column ที่เหลืออยู่ใน row ปัจจุบัน ให้ขึ้นบรรทัดใหม่
          for (let j = 1; j < col["colSpan"]; j++) {
            if (currentRow.length < maxColumnsPerRow) {
              currentRow.push({});
            }
          }
          pushOccupationContent()
        }
        currentRow.push(col);
        for (let j = 1; j < col["colSpan"]; j++) {
          if (currentRow.length < maxColumnsPerRow) {
  ...
  if (currentRow.length > 0) {
    ...
    occupationContent.push({
      ...
      padding: [0, 0, 0, 0],
      table: {
        widths: Array(maxColumnsPerRow).fill(maxColumnWidth), body: [currentRow]
      },
      layout: setting.layoutTableEmpty
    });
  }
  function pushOccupationContent() {
    if (currentRow.length === maxColumnsPerRow) {
      occupationContent.push({
        padding: [0, 0, 0, 0],
        table: {
          widths: Array(maxColumnsPerRow).fill(maxColumnWidth),
          body: [currentRow]
        },
        layout: setting.layoutTableEmpty
      });
...
function questionSectionMedia(collection) {
  ...
  let dataContent = genarateAnswers(csseen_media, part4?.Questions, 'csseen_media')
...
function questionSectionMediaOnlineOffline(collection) {
  ...
  let dataContentOnline = genarateAnswers(online_media, part4?.Questions, 'online_media')
  let dataContentOffline = genarateAnswers(offline_media, part4?.Questions, 'offline_media')
  ...
  return [{
    stack: [
      online_media || offline_media ? questionSectionHeader([
        {
      ...
      online_media ? contentOnline : null,
      offline_media ? contentOffline : null,
    ], unbreakable: true
  }]
...
function questionSection(collection, no, CRMInterface, PartNo) {
  let part = collection?.find(f => f.PartNo === PartNo);
  let data = part?.Questions?.find(f => f.CRMInterface === CRMInterface)
  let dataContent = genarateAnswers(data, part?.Questions, CRMInterface)
  if (!data) return []
  return [{
    stack: [
      questionSectionHeader(no + ". " + data?.QuestionTitle),
      ...(Array.isArray(dataContent) ? dataContent : [dataContent])
    ], unbreakable: true
  }]
}
let PaperColorHeader = '#000000';
function content(data) {
  PaperColorHeader = data.PaperColorHeader || PaperColorHeader;
  ...
  const partNoMap = {
    ...
    family_income: '2',
    marital_status: '2',
    objective_considering: '3',
    housing_characteristics: '3',
    csbudget: '3',
    visit_route: '4',
  };

  let sectionNo = 2;
  for (const [CRMInterface, PartNo] of Object.entries(partNoMap)) {
    const part = collection?.find(f => f.PartNo === PartNo);
    const dataQ = part?.Questions?.find(f => f.CRMInterface === CRMInterface);
    if (dataQ) {
      sections.push(questionSection(collection, sectionNo++, CRMInterface, PartNo));
    }
  }

  sections.push(questionSectionMedia(collection));
  sections.push(questionSectionMediaOnlineOffline(collection));
  sections.push(questionSection(collection, sections.length, "visit_route", '4'));
      ...
      body: [
...

        {
          ...
          table: {
            widths: [45, '*'],
            body: [
              [{ text: 'Ads Consent*', color: "#C92028", fontSize: 6, margin: [0, 1.5, 0, 0] }, { text: consent?.AdsConsentTextTH, fontSize: 6, margin: [0, 1.5, 0, 0] }],
            ]
          },
          layout: setting.layoutTableEmpty
        ...
        [checkbox(2),
        {
          ...
          table: {
            widths: [45, '*'],
            body: [
              [{ text: 'BC Consent*', color: "#C92028", fontSize: 6, margin: [0, 1.5, 0, 0] }, { text: consent?.BCConsentTextTH, fontSize: 6, margin: [0, 1.5, 0, 0] }],
            ]
          },
          layout: setting.layoutTableEmpty
...
function layout(data) {
  ...
  let half = Math.ceil(items.length / 2) + 2;
  let footerLayout = [
    consent ? consentContent(consent) : null,
    layoutFormDescription([textLabel("สำหรับเจ้าหน้าที่", '#B66612'), borderBottom('#B66612')], ['auto', '*'], [0, -4, 50, 5]),
    layoutFormDescription([borderBottom('#B66612')], ['*'], [0, 0, 50, 5]),
  ]
  items.push([...footerLayout])
  let leftCount = 7;
  if (items.length > leftCount) {
    return [
      {
        columns: [
          { stack: items.slice(0, half), width: 400 },
          { stack: items.slice(half), width: 400 }
        ],
        columnGap: 10
      }
    ];
