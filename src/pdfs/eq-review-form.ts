...
function genarateAnswers(data, maxCol?, part2Questions?, CRMInterface?) {
  ...
  let maxColumnWidth = 90; // ความกว้างสูงสุดต่อ column
  ...
  data?.Answers?.forEach((m, i) => {
    ...
    if (m.AnswerType == 2) {
      ...
      body = [checkbox(2, -4), layoutFormDescription(answerOther, ['auto', '*'], [-7, -2, 0, 0])]
      layout = {
        defaultBorder: false,
        hLineWidth: () => 0,
        vLineWidth: () => 0,
    ...
    if (CRMInterface == "marital_status" && i == data.Answers.length - 1) {
      ...
      body = [checkbox(2, 0), children_numb]
    }
